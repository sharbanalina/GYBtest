...
((SUM(total_revenue) - LAG(SUM(total_revenue)) OVER (ORDER BY DATE_TRUNC('month', order_date_kyiv))) / LAG(SUM(total_revenue)) OVER (ORDER BY DATE_TRUNC('month', order_date_kyiv)) * 100) AS growth_percent
